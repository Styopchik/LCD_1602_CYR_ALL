#ifndef _LCD_1602_CYR_ALL_h
#define _LCD_1602_CYR_ALL_h
...
class LCD_1602_CYR : public LCD_LIB {
  public:
#if (_LCD_TYPE == 1)
    LCD_1602_CYR (uint8_t lcd_Addr, uint8_t lcd_cols, uint8_t lcd_rows, uint8_t user_custom_symbols = 0) : LCD_LIB (lcd_Addr, lcd_cols, lcd_rows) { //Конструктор для подключения I2C
      init_priv(user_custom_symbols);
    }
#elif (_LCD_TYPE == 2)
    LCD_1602_CYR (uint8_t rs,  uint8_t enable,
      ...
      if (_chr < 128) //Английский алфавит без изменения
        ...
        switch (_chr)
        {
          ...
          default:
            ...
            memcpy_PF(cyr_, (_uint_farptr_t)font[getIndex(_chr)].getBitmap(), 8);
            CharSetToLCD((uint8_t *)cyr_, &font[getIndex(_chr)].index);
            break;
        }
    }
